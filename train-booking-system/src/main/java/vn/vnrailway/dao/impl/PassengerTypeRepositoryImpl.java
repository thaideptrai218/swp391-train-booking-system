...
public class PassengerTypeRepositoryImpl implements PassengerTypeRepository {
    ...
    @Override
    public List<PassengerType> findAllOrderByDiscountPercentage() throws SQLException {
        List<PassengerType> passengerTypes = new ArrayList<>();
        String sql = "SELECT PassengerTypeID, TypeName, DiscountPercentage, Description, RequiresDocument FROM PassengerTypes ORDER BY DiscountPercentage ASC";
        try (Connection conn = DBContext.getConnection();
                PreparedStatement ps = conn.prepareStatement(sql);
                ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
