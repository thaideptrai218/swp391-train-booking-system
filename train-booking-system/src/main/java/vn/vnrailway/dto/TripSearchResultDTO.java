...
public class TripSearchResultDTO {
    ...
    private LocalDateTime scheduledDeparture; // Scheduled departure time from the origin station for this leg - from
                                              // SP: DepartureTime

    private String destinationStationName; // Name of the destination station for this leg - from SP: DestinationStation
    private LocalDateTime scheduledArrival; // Scheduled arrival time at the destination station for this leg - from SP:
                                            // ArrivalTime
    ...
    // if available from joins in SP)
    // These might not be directly in the final SELECT of the SP but are part of the
    // joined tables.
    // If they are needed, the SP would have to be modified to select them, or they
    // are populated by the service layer.
    // For now, assuming they might be populated by the service if the IDs are
    // fetched separately or available.
