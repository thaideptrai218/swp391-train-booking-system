            ...
            <div class="chat-container">
                <div class="chat-list">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <c:if test="${empty chatSummaries}">
                                <tr>
                                    <td colspan="4" style="text-align: center;">Không có tin nhắn nào từ khách hàng.</td>
                                </tr>
                            </c:if>
                            <c:forEach var="summary" items="${chatSummaries}" varStatus="loop">
                                <tr>
                                    ...
                                    <td>
                                        <form action="${pageContext.request.contextPath}/staff-message" method="get" style="display: inline;">
                    ...
                    <div class="pagination">
                        <c:if test="${currentPage > 1}">
                            <a href="${pageContext.request.contextPath}/staff-message?page=${currentPage - 1}">Previous</a>
                        </c:if>
                        <c:forEach begin="1" end="${totalPages}" var="page">
                            <c:choose>
                                <c:when test="${page == currentPage}">
                                    <a href="${pageContext.request.contextPath}/staff-message?page=${page}" class="active">${page}</a>
                                </c:when>
                                <c:otherwise>
                                    <a href="${pageContext.request.contextPath}/staff-message?page=${page}">${page}</a>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                        <c:if test="${currentPage < totalPages}">
                            <a href="${pageContext.request.contextPath}/staff-message?page=${currentPage + 1}">Next</a>
                        </c:if>
                    </div>
                </div>
                <c:if test="${not empty param.userId}">
                    ...
                    <div class="chat-box" id="chatBox">
                        ...
                        <form action="${pageContext.request.contextPath}/staff-message" method="post" id="chatForm">
                            <input type="hidden" name="userId" value="${param.userId}" />
                            <div class="chat-input">
                                <textarea name="message" id="chatInput" placeholder="Nhập tin nhắn..." rows="3" required></textarea>
    ...
    <c:if test="${not empty param.userId}">
        <script>
            ...
            $(document).ready(function () {
                ...
                fetchNewMessages(); // Call for the first time
